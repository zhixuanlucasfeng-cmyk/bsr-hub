    ...
    <main className="mx-auto max-w-[1320px] px-5 py-10 sm:px-8 lg:px-12 lg:py-16">
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,7fr)_minmax(320px,3fr)] lg:items-start">
        <div>
          ...
          <div className="mt-9"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-violet-100 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.1em] text-brand">{listing.listingType === "workspace" ? "Creative space" : listing.listingType === "sale" ? "Second-hand" : "Rental"}</span><span className="rounded-full bg-zinc-100 px-3 py-1.5 text-[10px] font-bold text-zinc-600">{listing.condition}</span></div><h1 className="mt-4 font-[Manrope] text-4xl font-bold leading-tight tracking-[-.04em] text-zinc-950 sm:text-5xl">{listing.title}</h1><p className="mt-3 text-sm font-semibold text-brand">{listingPriceLabel(listing)}</p></div>
