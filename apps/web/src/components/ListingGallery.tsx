...
export function ListingGallery({ listing }: { listing: Listing }) {
  ...
  return <div>
    <div className="group relative aspect-[16/11] touch-pan-y overflow-hidden rounded-[24px] bg-zinc-100 shadow-soft" onPointerDown={(event) => { startX.current = event.clientX; }} onPointerUp={(event) => finishSwipe(event.clientX)} onPointerCancel={() => { startX.current = null; }}>
      {images.map((image, index) => <OptimizedImage key={image.src} source={image.src} alt={image.alt} mode="detail" eager={index === 0} className={`absolute inset-0 size-full object-contain transition-opacity duration-300 ${index === active ? "opacity-100" : "pointer-events-none opacity-0"}`}/>)}
    ...
    {images.length > 1 && <div className="mt-4 flex gap-3 overflow-x-auto pb-2">{images.map((image, index) => <button key={image.src} onClick={() => setActive(index)} aria-label={`View image ${index + 1}`} className={`relative aspect-[4/3] w-24 shrink-0 overflow-hidden rounded-xl transition ${index === active ? "ring-3 ring-brand ring-offset-2" : "opacity-65 hover:opacity-100"}`}><OptimizedImage source={image.src} alt="" className="absolute inset-0 size-full object-cover"/></button>)}</div>}
  </div>;
}
