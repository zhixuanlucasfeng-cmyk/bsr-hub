  ...
  return <section className="relative isolate overflow-hidden px-5 pb-20 pt-32 sm:px-8 lg:px-12 lg:pb-28 lg:pt-40">
    ...
    <div className="mx-auto grid max-w-[1320px] items-center gap-16 lg:grid-cols-[1.02fr_.98fr]">
      <div>
        ...
        <h1 className="max-w-3xl font-[Manrope] text-6xl font-bold leading-[.94] tracking-[-.065em] text-zinc-950 sm:text-7xl lg:text-[92px]">Use more.<br/><span className="bg-gradient-to-r from-brand to-violet-500 bg-clip-text text-transparent">Own less.</span></h1>
      ...
      <div className="relative mx-auto h-[430px] w-full max-w-[610px] sm:h-[520px]">
        ...
        {scenes.map((scene) => <article key={scene.label} className={`group absolute overflow-hidden rounded-[24px] bg-white p-2 shadow-[0_25px_70px_rgba(30,18,55,.24),0_4px_15px_rgba(30,18,55,.12)] transition duration-200 hover:z-10 hover:-translate-y-2 hover:rotate-0 ${scene.className}`}>
          <div className="relative aspect-[4/3] overflow-hidden rounded-[18px]"><img src={`${basePath}${scene.src}`} alt={scene.alt} className="size-full object-cover transition duration-300 group-hover:scale-105"/><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-4 pt-12 text-white"><strong className="block text-sm">{scene.label}</strong><span className="mt-1 inline-block rounded-full bg-accent px-2.5 py-1 text-[10px] font-extrabold text-zinc-950">{scene.price}</span></div></div>
        </article>)}
      </div>
    </div>
  </section>;
