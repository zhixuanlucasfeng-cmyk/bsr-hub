...
export function FeaturedListings({ listings, query, listingType, onQuery, onType }: FeaturedListingsProps) {
  return <section id="market" className="scroll-mt-24 px-5 py-20 sm:px-8 lg:px-12">
    <div className="mx-auto max-w-[1320px]">
      ...
      {listings.length ? <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">{listings.map((listing, index) => <MarketplaceListingCard key={listing.id} listing={listing} eager={index < 4}/>)}</div> : <div className="rounded-card bg-white p-16 text-center text-zinc-500 shadow-soft">No listings match these filters.</div>}
    </div>
  </section>;
}
