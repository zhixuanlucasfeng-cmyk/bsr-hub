...
export function MarketplaceListingCard({ listing, eager = false }: MarketplaceListingCardProps) {
  ...
  return <article className="group relative overflow-hidden rounded-card bg-white shadow-soft transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_30px_75px_rgba(27,18,48,.16),0_5px_18px_rgba(27,18,48,.1)]">
    <Link href={`/listings/${listing.id}/`} className="block focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-brand" aria-label={`View ${listing.title}`}>
      <div className="relative aspect-[4/3] overflow-hidden bg-zinc-100"><img src={`${basePath}${listing.imageSrc}`} alt={listing.imageAlt} loading={eager ? "eager" : "lazy"} className="size-full object-cover transition duration-300 group-hover:scale-[1.045]"/><span className="absolute left-3 top-3 rounded-full bg-white/92 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.1em] text-zinc-900 shadow-sm backdrop-blur">{typeLabel}</span><span className="absolute inset-x-3 bottom-3 translate-y-4 rounded-full bg-zinc-950/88 py-2.5 text-center text-xs font-bold text-white opacity-0 backdrop-blur transition group-hover:translate-y-0 group-hover:opacity-100">View & reserve</span></div>
      <div className="p-4.5">
        <p className="text-[10px] font-bold uppercase tracking-[.12em] text-brand">{listing.category} · {listing.condition}</p>
        <h3 className="mt-2 min-h-12 font-[Manrope] text-[17px] font-bold leading-6 text-zinc-950">{listing.title}</h3>
        ...
        <div className="mt-4 flex items-end justify-between border-t border-zinc-100 pt-4"><strong className="font-[Manrope] text-lg font-bold text-brand">{listingPriceLabel(listing)}</strong><span className="flex items-center gap-1 text-[10px] font-semibold text-zinc-500"><LinearIcon name="truck" className="size-3.5"/>{distance.toFixed(1)} mi</span></div>
      </div>
    </Link>
  </article>;
}
